...
// Parse URL-encoded bodies (for FormData without files)
...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    ...
    const ext = path.extname(file.originalname);
    const allowedMimes =
      /application\/(pdf|msword|vnd.openxmlformats-officedocument|vnd.ms-excel)|text\/(plain|csv)|image\/(jpeg|png)|application\/x-zip/i;

    if (allowedExt.test(ext) && allowedMimes.test(file.mimetype)) {
      cb(null, true);
    } else {
